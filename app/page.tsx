...
const LandQueryAssistant = () => {
  ...
  const [response, setResponse] = useState("");
  const [isListening, setIsListening] = useState(false);
  const [voices, setVoices] = useState<SpeechSynthesisVoice[]>([]); // eslint-disable-line @typescript-eslint/no-unused-vars
  const [selectedLanguage, setSelectedLanguage] = useState("en");
//   const [audioUrl, setAudioUrl] = useState<string | null>(null); // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  const startListening = () => {
    ...
    const recognition = new (window as typeof window & { webkitSpeechRecognition: new () => SpeechRecognition }).webkitSpeechRecognition();
    recognition.continuous = false;
